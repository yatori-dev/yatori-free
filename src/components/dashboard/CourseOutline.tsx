      ...
      <div className={isFullyExpanded ? undefined : 'max-sm:max-h-64 max-sm:overflow-hidden'}>
        ...
        <div className="grid grid-cols-1 gap-2 pr-1 md:max-h-[300px] md:grid-cols-2 md:overflow-y-auto">
          {chaptersWithTasks.map(({ chapter: chapterItem, taskMeta, taskPoints }) => {
            const chapterFromSource = chapterItems.find((chapter) => String(chapter.id) === String(chapterItem.id));
            const sourceTaskMeta = chapterFromSource ? getChapterTaskMetas([chapterFromSource])[0]?.taskMeta : undefined;
            const hasKnownTaskPointStatus = taskPoints.some((taskPoint) => taskPoint.completed !== undefined);
            const resolvedTaskMeta = hasKnownTaskPointStatus || !sourceTaskMeta?.hasTaskPoints ? taskMeta : sourceTaskMeta;
            const isChapterDone = !resolvedTaskMeta.isLocked && resolvedTaskMeta.total > 0 && resolvedTaskMeta.finished === resolvedTaskMeta.total;
            const chapterDocuments = getChapterDocuments(chapterItem, courseDetails.documents);
            const statusClassName = resolvedTaskMeta.isLocked ? 'border-border bg-muted text-muted-foreground' : isChapterDone ? 'border-success/20 bg-success-container text-success' : 'border-warning/20 bg-warning-container text-warning';
            return <div key={chapterItem.id} className="rounded-lg border border-border/60 bg-card p-3 text-xs shadow-xs transition-[border-color,box-shadow] duration-200 hover:border-primary/30 hover:shadow-sm">
              <div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><span className="mr-1.5 font-semibold text-muted-foreground">{chapterItem.label}</span><span className="font-medium text-foreground">{chapterItem.name}</span></div><Badge className={`shrink-0 border text-xs font-normal ${statusClassName}`}>{taskPoints.length > 0 ? `任务点: ${resolvedTaskMeta.finished}/${resolvedTaskMeta.total}` : resolvedTaskMeta.isLocked ? `未开放任务点: ${resolvedTaskMeta.total}` : `任务点: ${resolvedTaskMeta.finished}/${resolvedTaskMeta.total}`}</Badge></div>
              ...
              {accountId && chapterDocuments.length > 0 && <div className="mt-2 space-y-1.5 border-t border-border/50 pt-2">{chapterDocuments.map((document) => { const fileSize = formatFileSize(document.size); const fileName = getCourseDocumentFileName(document); return <div key={document.id} className="flex items-center gap-2 rounded-md bg-muted/40 px-2.5 py-1.5 transition-colors hover:bg-muted/70"><FileText className="h-3.5 w-3.5 shrink-0 text-muted-foreground" /><div className="min-w-0 flex-1"><div className="truncate font-medium text-foreground">{document.name}</div><div className="text-xs text-muted-foreground">{getCourseDocumentTypeLabel(document)}{fileSize ? ` · ${fileSize}` : ''}</div></div><Button asChild variant="ghost" size="sm" className="h-7 w-7 shrink-0 rounded p-0 text-primary hover:bg-primary/10 transition-colors"><a href={getCourseDocumentDownloadUrl(accountId, courseKey, document.id)} download={fileName} aria-label={`下载 ${document.name}`}><Download className="h-3.5 w-3.5" /></a></Button></div>; })}</div>}
            </div>;
          })}
