...
export function DashboardOverlays({ selectedCount, creatingTask, estimatedTaskDuration, nightConfirmOpen, submitBypassConfirmOpen, logoutConfirmOpen, studyIncrementCourseKey, studyIncrementCourse, studyIncrementCourseDetails, loadingDetails, studyIncrements, onCreateTask, onNightConfirmChange, onSubmitBypassConfirmChange, onLogoutConfirmChange, onConfirmNightTask, onExecuteSubmitTask, onStudyIncrementOpenChange, onSaveStudyIncrement, onLogout }: DashboardOverlaysProps) {
  return <>
    {selectedCount > 0 && <div className="absolute bottom-[calc(4.5rem+env(safe-area-inset-bottom))] left-1/2 z-50 -translate-x-1/2 animate-bottom-bar-enter lg:bottom-6"><div className="flex flex-col items-center gap-1"><Button type="button" onClick={onCreateTask} disabled={creatingTask} className="h-11 gap-2 rounded-full bg-primary px-4 text-sm font-semibold text-primary-foreground shadow-floating ring-4 ring-card/80 hover:bg-primary-hover" title={`提交 ${selectedCount} 门课程任务`} aria-label={`提交 ${selectedCount} 门课程任务`}>{creatingTask ? <RefreshCw className="h-4 w-4 animate-spin" aria-hidden="true" /> : <Play className="h-4 w-4 fill-current" aria-hidden="true" />}<span>提交任务({selectedCount})</span></Button>{estimatedTaskDuration && <span className="whitespace-nowrap text-[11px] font-medium text-muted-foreground" role="status">预计所需{estimatedTaskDuration}</span>}</div></div>}
